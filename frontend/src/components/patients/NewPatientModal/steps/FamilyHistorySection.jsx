...
const FamilyHistorySection = ({ formData, setFormData }) => {
  // 🌟 FUNCIÓN PROTEGIDA CONTRA UNDEFINED
  const updateFamily = (id, field, value) => {
    // Si 'family' no existe, usamos un objeto vacío {}
    const currentFamily = formData.medicalHistory.family || {};
    // Si la enfermedad específica no existe, usamos un objeto vacío {}
    const currentCondition = currentFamily[id] || {};
    ...
    setFormData({
      ...
      medicalHistory: {
        ...
        family: {
          ...currentFamily,
          [id]: { ...currentCondition, [field]: value },
        },
      },
    });
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <header className="p-4 bg-amber-50 rounded-4xl border border-amber-100 mb-6">
      ...
      {conditions.map((item) => {
        ...
        const hasCondition = formData.medicalHistory.family?.[item.id]?.has;
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl w-fit">
                {[true, false, "DESCONOZCO"].map((v) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[9px] font-black transition-all ${
                      hasCondition === v
              ...
              <div className="mt-4 flex flex-col gap-3 animate-in slide-in-from-top-2 duration-300">
                ...
                {item.id === "cancer" && (
                  <input
                    ...
                    value={
                      formData.medicalHistory.family?.[item.id]?.type || ""
                    }
                    onChange={(e) =>
